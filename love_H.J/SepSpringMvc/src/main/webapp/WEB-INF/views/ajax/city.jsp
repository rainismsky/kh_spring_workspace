...
<%
    String cp = request.getContextPath();// /springMvc ContextPath를 구해서 cp변수에 넣는다. 프로그램에 접근할 수 있는 contextroot
...
<script>
$(function(){//바디로딩이 다끝난다음에 실행
	// 시도테이블의 리스트 가져오기

	var url="<%=cp%>/city/sidoList.do";
	var params="dumi="+new Date();//쿼리 문자열 형식
	...
	$.ajax({
		...
		,dataType:"json"
		,success:function(args){	//응답이 성공 상태 코드를 반환하면 호출되는 함수 args에 response에 저장된 데이터가 첫번째 변수에 저장이 된다.
		...
		$.ajax({
			...
			success : function(args) {
				...
				for (var idx = 0; idx < args.data1.length; idx++) { // 새로 가져온 데이터를 데이터 갯수만큼 반복해서 붙여준다.
					var area2 = args.data1[idx];//area2(맵객체)
				console.log(area2);

					$("#city").append(
							"<option value='"+area2['AREA2']+"'>"
									+ area2['AREA2'] + "</option>");
