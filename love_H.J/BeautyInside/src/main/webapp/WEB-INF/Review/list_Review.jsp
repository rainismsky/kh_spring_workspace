...
<script>
 $(document).ready(function() {
	$("#R_detail").click(function() {
	//페이지 주소 변경(이동)
	location.href = "${pageContext.request.contextPath}/Detail_Review";
	});
});
</script>
</head>
<body>
	...
	<table border="1" width="800px">
		...
		<c:forEach var="dto" items="${list}">
			<tr>
				<td>${dto.rno}</td>
				<td><a href="<c:url value='/Review/Detail_Review?rno=${dto.rno}'/>">${dto.rcontents}</a></td>
				<td>${dto.mid}</td>
				<td>${dto.r_regdate}</td>
				<td>${dto.r_grade}</td>
				
				<%-- <fmt:formatDate value="${dto.r_regdate}" pattern="yyyy-MM-dd HH:mm:ss"/> --%>
